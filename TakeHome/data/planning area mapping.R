...
dgp <-  centroids <- readOGR("MP14_PLNG_AREA_WEB_PL.shp")
...
leaflet(dgp) %>% addTiles() %>% addPolygons(fillColor = ~pal(m$Units),
                                                                            ...
                                                                            color = "grey",
                                                                            dashArray = "1",
                                                                            fillOpacity = 0.8) %>% addLegend("topright", pal, values=(0:2000), title = "Transacted", labFormat = labelFormat(suffix = " Units", between = '-'))
